...
bool Sprite::Initialize(LPDIRECT3DDEVICE9 device, LPCWSTR file)
{

	if (LoadTexture(device, file) == NULL)
	{
		//LPCSTR file_ = file;
		MessageBox(NULL, file, L"Error", MB_OK);
		return initialized;
	}
	...
	if (FAILED(result))
	{
		MessageBox(NULL, L"Error creating Direct 3D sprite", L"Error", MB_OK);
		return initialized;
	}
...
bool Sprite::Initialize(LPDIRECT3DDEVICE9 device, LPCWSTR file, int width, int height)
{

	if (LoadTexture(device, file, width, height) == NULL)
	{
		MessageBox(NULL, L"Could not load image file", L"Error", MB_OK);
		return initialized;
	}
...
//load Texture
LPDIRECT3DTEXTURE9 Sprite::LoadTexture(LPDIRECT3DDEVICE9 device, LPCWSTR file)
{
	D3DXIMAGE_INFO info;
	HRESULT result;
...
LPDIRECT3DTEXTURE9 Sprite::LoadTexture(LPDIRECT3DDEVICE9 device, LPCWSTR file, int width, int height)
{
	HRESULT result;
	this->width = width;
	this->height = height;
...
void Sprite::Render()
{
	if (spriteHandle && texture)
	{
		...
		spriteHandle->Draw(texture, NULL, NULL,&position, color);
		spriteHandle->End();
	}
}
...
void Sprite::Render(RECT rect, bool isLeft, int width, int height)
{
	if (spriteHandle && texture)
	{
		...
		D3DXVECTOR2 center = D3DXVECTOR2(position.x + width / 2, position.y + height / 2);
		...
		spriteHandle->Draw(texture, &rect, NULL, &position, color);
...
void Sprite::Render(D3DXVECTOR3 position, D3DCOLOR color)
{
	if (spriteHandle && texture)
	{
		spriteHandle->Begin(D3DXSPRITE_ALPHABLEND);
